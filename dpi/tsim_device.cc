...
void VTASimDPI(dpi8_t *wait, dpi8_t *exit) {
    assert(_sim_dpi != nullptr);
    (*_sim_dpi)(_ctx, wait, exit);
}

void VTAHostDPI(dpi8_t *req_valid, dpi8_t *req_opcode, dpi16_t *req_addr,
                dpi32_t *req_value, dpi8_t req_deq, dpi8_t resp_valid,
                dpi32_t resp_value) {
    assert(_host_dpi != nullptr);
    (*_host_dpi)(_ctx, req_valid, req_opcode, req_addr, req_value, req_deq,
                 resp_valid, resp_value);
}

void VTAMemDPI(dpi8_t req_valid, dpi8_t req_opcode, dpi8_t req_len,
    ...
    (*_mem_dpi)(_ctx, req_valid, req_opcode, req_len, req_addr, wr_valid,
                wr_value, rd_valid, rd_value, rd_ready);
}

void VTADPIInit(VTAContextHandle handle, VTASimDPIFunc sim_dpi,
