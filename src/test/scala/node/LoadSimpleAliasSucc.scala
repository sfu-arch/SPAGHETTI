...
class LoadAliasSuccTests(c: UnTypLoadAlias) extends PeekPokeTester(c) {
  def N = false
  ...
  poke(c.io.OutA.SuccOp(0).ready, true.B)
  poke(c.io.OutA.Out(0).ready, true.B)
  // Enable1.valid,
  // Enable1.control,
  // Input1.valid,
  // Input1.data,
  ...
  for (t <- 0 until 40) {
    ...
    if ((peek(c.io.InA.In(0).ready) == 1) && (t > 4)) {
      str = "active"
      aliassigs zip AliasControl("Active") map { case (s, d) => poke(s, d) }
      poke(c.io.InA.In(0).valid, 1)
      poke(c.io.InA.In(1).valid, 1)
      poke(c.io.InA.In(0).bits.data, 12)
      poke(c.io.InA.In(1).bits.data, 6)
    }

    if (t > 8) {
      poke(c.io.InA.PredOp(0).valid, Y)
    }
    ...
    print(s"t: ${t}  io.Out: ${peek(c.io.OutA.Out(0))} \n")
