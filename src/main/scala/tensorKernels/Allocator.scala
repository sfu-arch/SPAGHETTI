...
class AllocatorIO(numIns: Int, numOuts: Int)(implicit val p: Parameters) extends Module {
...
class Allocator(numIns: Int, numOuts: Int)(implicit p: Parameters)
  extends AllocatorIO(numIns, numOuts)(p) {

  val arbiter = for (i <- 0 until numOuts) yield {
    val arb = Module(new RRArbiter(new CooDataBundle(UInt(p(XLEN).W)) ,n = numIns))
    arb
  }
