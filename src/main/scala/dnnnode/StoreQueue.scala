...
//  io.enq.ready := !full
  ...
  for (i <- 0 until NumOuts) {
    io.deq.bits(i) := Mux(bufCount > (NumOuts - 1).U, ram((deq_ptr.value + i.U) % entries.U),
      Mux(i.U >= bufCount, 0.U.asTypeOf(genType), ram((deq_ptr.value + i.U) % entries.U)))

//    io.deq.bits(i) := Mux(bufCount > (NumOuts - 1).U, ram((deq_ptr.value + i.U) % entries.U), 0.U.asTypeOf(genType))
  }
