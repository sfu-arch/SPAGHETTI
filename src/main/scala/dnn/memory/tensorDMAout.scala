...
import dnnnode.{MIMOQueue, StoreQueue}
...
/** outDMA_act
  ...
  */
class tensorDMA_outIO(NumRows: Int, NumElem: Int, memTensorType: String = "none")(implicit val p: Parameters)
  extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val in = Vec(NumRows, Flipped(Decoupled(new CustomDataBundle(UInt((p(XLEN) * NumElem).W)))))
    val last = Vec(NumRows, Input(Bool()))
  })
}

class tensorDMA_out[L <: Shapes](NumRows: Int, bufSize: Int, memTensorType: String = "none")(vecShape: => L)(implicit p: Parameters)
  extends tensorDMA_outIO(NumRows, vecShape.getLength(), memTensorType)(p) {
  ...
  val storeBuffer = for (i <- 0 until NumRows) yield {
    val buf = Module(new MIMOQueue(UInt(p(XLEN).W), bufSize, vecShape.getLength(), tp.tensorWidth))
    buf
  }
  ...
  for (i <-0 until NumRows) {
    ...
    storeBuffer(i).io.clear := io.last(i)
    storeBuffer(i).io.enq.bits := VecInit(io.in(i).bits)
    storeBuffer(i).io.enq.valid := io.in(i).valid
    io.in(i).ready := storeBuffer(i).io.enq.ready

    storeBuffer(i).io.deq.ready := true.B
    tensorStore(i).io.tensor.wr.valid := storeBuffer(i).io.deq.valid
    tensorStore(i).io.tensor.wr.bits.data := 0.U//VecInit(storeBuffer(i).io.deq.bits).asTypeOf(tensorStore(i).io.tensor.wr.bits.data)
