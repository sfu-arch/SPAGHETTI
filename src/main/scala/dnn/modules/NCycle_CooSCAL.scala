...
class NCycle_CooSCAL[T <: Data : TwoOperand.OperatorTwoOperand](val gen: T, val N: Int, val lanes: Int, val opcode: String)(implicit val p: Parameters)
  ...
  val io = IO(new Bundle {
    val input_vec = Input(Vec(N, Valid(UInt(xlen.W))))
    val scalar    = Input(Valid(UInt(xlen.W)))
    val output    = Output(Vec(lanes, Valid(UInt(xlen.W))))
  })
  ...
  for (i <- 0 until lanes) {
    PEs(i).io.left.bits := io.input_vec(i).bits
    PEs(i).io.right.bits := io.scalar
    PEs(i).io.left.valid := io.input_vec(i).valid
    PEs(i).io.right.valid := io.scalar.valid
  }

  for (i <- 0 until lanes) {
    io.output(i) <> PEs(i).io.out
    PEs(i).reset := false.B
  }
}