...
$(chisel_build_dir)/$(TOP).v: $(shell find . -type f -name "*.scala" | sed 's/ /\\ /g')#install_vta_package
	sbt 'test:runMain accel.${TOP}Main --target-dir $(chisel_build_dir) --top-name $(TOP) --numSegment $(NUM_Segment) --numSorter $(NUM_Sorter) --numColMerger $(NUM_ColMerger) --numVC $(NUM_VC) --VCDepth $(VCDepth) --maxRowLen $(MAX_RowLen) --maxColLen $(MAX_ColLen)'
